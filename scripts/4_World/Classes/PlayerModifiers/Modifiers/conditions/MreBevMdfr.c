class MreBevMdfr: ModifierBase
{
	private const float MRE_BEV_BUFF_COEF = 2;
	...
	override void Init()
	{
		...
		m_IsPersistent			= true;
		m_ID 					= MRTsMRE_Modifiers.MDF_MRE_BEV;
	...
	{
		if(player.GetSingleAgentCount(MRTsMRE_Agents.MRE_BEV) >= AGENT_THRESHOLD_ACTIVATE) 
		{
			return true;
		}
		else 
	...
	{
		if(player.GetSingleAgentCount(MRTsMRE_Agents.MRE_BEV) <= AGENT_THRESHOLD_DEACTIVATE) 
		{
			return true;
		}
		else 
